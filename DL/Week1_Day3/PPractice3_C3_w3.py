...
x = data.iloc[0:1000, 3:13]
y = data.iloc[0:1000, 13]
...
x_hot_en = onehotencoder.fit_transform(x).toarray()
x_hot_en = x_hot_en[:, 1:]
...
x_train, x_test, y_train, y_test = train_test_split(x_hot_en, y, test_size = 0.2,\
                                                    random_state = 0)
